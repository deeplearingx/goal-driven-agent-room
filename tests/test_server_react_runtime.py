...
def test_default_spec_developer_is_tool_using(tmp_path: Path):
    ...
    assert dev.tools == ["glob", "read_text", "write_text", "shell", "memory"]
...
def test_graph_preset_override_selects_goal(tmp_path: Path):
    ...
    assert "shell" in goal_spec.nodes["developer"].tools
...
def test_describe_envelope_reports_active_config(tmp_path: Path):
    ...
    assert env["shell_enabled"] is True
    assert env["tool_mode"] == "unrestricted"
...
def test_parse_allowlist_unset_uses_default():
    assert _parse_allowlist(None) == ("pytest", "python", "python3", "ruff")
