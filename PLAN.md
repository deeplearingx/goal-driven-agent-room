...
## Enterprise Control Plane Workstream (2026-07)

**Status:** active on `feature/enterprise-control-plane`.

The project is evolving from a task execution service into an enterprise Agent
platform. The rollout is intentionally incremental: the existing Python
LangGraph worker remains a compatible executor while the Go control plane and
future Eino runtime are introduced behind stable command/event contracts.

### P0: tenancy, authorization and audit foundation

- replace the single repeatable schema script with ordered, recorded Postgres
  migrations;
- add tenants, principals, role bindings and append-only audit events;
- introduce authenticated actor context and RBAC before exposing admin APIs;
- retain the current API-key path only as a development/service-account bridge;
- add ADR-0014 and integration tests for tenant isolation and audit coverage.

### Follow-on milestones

1. Immutable Agent, Prompt, Tool, Skill and Model Profile versions, plus a
   runtime snapshot resolved once per task.
2. Go/Eino runtime parity spike, ReAct event contract and dual-run cutover.
3. Unified tools, MCP/A2A, approval policies and isolated code execution.
4. Knowledge-base ingestion, pgvector/FTS/RRF retrieval and async workers.
5. Guardrails, audit, cost attribution, evaluations and release gates.
6. Admin console, Webhook/Feishu, Go SDK/CLI and Redis invalidation.
7. Production SLO, restore, security and load-test acceptance.

### Delivery ledger

| Phase | Status | Acceptance evidence |
| --- | --- | --- |
| P0 tenancy / auth / audit | complete | Ordered migrations, OIDC + tenant RBAC, append-only audit and isolation coverage. |
| P1 immutable control plane | complete | Versioned Agent/Prompt/Tool/Skill/Model resources, encrypted provider accounts, frozen runtime snapshots and Prompt rollout. |
| P2 Go/Eino runtime | complete | Runtime-partitioned quorum queues, Python compatibility worker, tagged Eino ReAct worker and common event contract. |
| P3 tool governance | complete | REST/MCP/A2A adapters, policy evaluation, human approval and isolated Docker sandbox execution. |
| P4 knowledge loop | complete | Async ingest worker, deterministic chunks, PostgreSQL FTS + pgvector, hybrid RRF search and frozen embedding profiles. |
| P5 safety / cost / eval | complete | Guard findings, invocation ledger, real token and estimated-cost attribution for Python and Eino, eval runs and Prompt release gates. |
| P6 enterprise integrations | complete | Control Plane Console covers runtime, immutable configuration, release/provider boundaries, Webhook delivery controls, approvals and audit; dependency-free Go SDK/`kbctl`, encrypted reliable Webhook delivery/Feishu bots, and Redis-backed configuration cache invalidation are available. |
| P6.5 model execution safety | complete | Safe read-only defaults, explicit direct-shell escape hatch, tenant/task path validation, one-call-per-approval serialization, digest-pinned remote sandbox profiles, server-derived workspaces and hardened Docker isolation are covered by regression tests and `docs/sandbox-security.md`. |
| P7 concurrency acceptance | in progress | Persistent outbox backoff, bounded tenant limiters, admission and idempotency profiles are implemented. Local isolated Compose evidence covers k6 idempotency/admission and RabbitMQ outage recovery; multi-replica managed-cluster soak/rolling-restart evidence remains. |
| P8 production acceptance | in progress | Production service images, PostgreSQL restore drill and Gateway SBOM were exercised locally; CI vulnerability-gates all service images and exports release evidence. Trivy database retrieval and the sandbox-runner runtime image pull are currently blocked by the local network mirror, and final SLO sign-off remains. |
