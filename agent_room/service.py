...
_MAX_GOAL_ITERATIONS = 50
# At every third failed verification (except the terminal ceiling), the
# worst-case supervisor path is supervisor -> planner: two extra supersteps.
# The previous formula omitted these 32 legal steps, so a difficult 50-round
# goal could raise GraphRecursionError before max_iterations produced a clean
# status="failed" result.
_MAX_SUPERVISOR_REPLAN_STEPS = 2 * ((_MAX_GOAL_ITERATIONS - 1) // 3)
GRAPH_RECURSION_LIMIT = (
    _FIXED_OVERHEAD
    + (max(10, _MAX_GOAL_ITERATIONS) + 1) * (_REACT_TURN_STEPS + 1)
    + _MAX_SUPERVISOR_REPLAN_STEPS
)
    ...
    async def resume(
        ...
        *,
        ...
        - `at_node="tool_call"`: resumes from a `tool_mode: approval` halt.
          ...
          Accepted values:
            * `"approve"` / `"deny"` (string) — apply the verdict to the
              single pending tool call.
            * `{"action": "deny", "message": "..."}` — custom denial body.
          Approval-mode model output is serialized before ToolNode: only one
          concrete call can be pending or approved at a time. Extra calls
          must be requested again after the first result is observed.
