...
def make_developer_react(
    ...
    *,
    ...
    async def developer_react(state: TaskState) -> dict[str, Any]:
        ...
        serialized_event: Event | None = None
        if serialize_tool_calls and len(response.tool_calls) > 1:
            # One approval must authorize exactly one concrete operation.
            # The model may request deferred calls again after it observes
            # the retained call's result.
            deferred = response.tool_calls[1:]
            response = response.model_copy(update={"tool_calls": response.tool_calls[:1]})
            serialized_event = Event(
                type="parallel_tool_calls_serialized",
                role="developer",
                round=round_no,
                payload={
                    "approved_scope": "single_tool_call",
                    "retained_id": response.tool_calls[0].get("id"),
                    "deferred_count": len(deferred),
                    "deferred_ids": [call.get("id") for call in deferred],
                },
            )
