...
CREATE TABLE IF NOT EXISTS agent_room_sessions (
    ...
    graph_preset TEXT NOT NULL DEFAULT 'full_react'
);
...
class SessionStore:
    ...
    async def initialize(self, db_path: str) -> None:
        ...
        with contextlib.suppress(aiosqlite.OperationalError):
            await self._conn.execute(
                "ALTER TABLE agent_room_sessions ADD COLUMN graph_preset TEXT NOT NULL DEFAULT 'full_react'"
            )
    ...
    async def create(
        ...
        now = time.time()
        await self._conn.execute(
            "INSERT INTO agent_room_sessions "
            "(id, name, created_at, tenant_id, graph_preset) VALUES (?, ?, ?, ?, ?)",
            (sid, clean, now, tenant_id, graph_preset),
        )
        await self._conn.commit()
        return SessionRecord(
            id=sid, name=clean, created_at=now, tenant_id=tenant_id, graph_preset=graph_preset
        )
    ...
    async def get(self, session_id: str) -> SessionRecord | None:
        ...
        async with self._conn.execute(
            "SELECT id, name, created_at, tenant_id, graph_preset "
            "FROM agent_room_sessions WHERE id = ?",
            (session_id,),
        ) as cursor:
        ...
        if row is None:
            return None
        return SessionRecord(
            id=row[0], name=row[1], created_at=row[2], tenant_id=row[3], graph_preset=row[4]
        )
    ...
    async def list_all(self, *, tenant_id: str | None = None) -> list[SessionRecord]:
        ...
        if tenant_id is None:
            query = (
                "SELECT id, name, created_at, tenant_id, graph_preset "
                "FROM agent_room_sessions ORDER BY created_at DESC"
            )
            params: tuple[str, ...] = ()
        else:
            query = (
                "SELECT id, name, created_at, tenant_id, graph_preset "
                "FROM agent_room_sessions "
        ...
        return [
            SessionRecord(
                id=r[0], name=r[1], created_at=r[2], tenant_id=r[3], graph_preset=r[4]
            )
            for r in rows
        ]
