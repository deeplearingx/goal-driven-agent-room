    ...
    *,
    ...
    async def task_service_for(
        ...
        registry = build_server_registry(
            settings,
            memory_provider=mem,
            workspace_override=task_workspace(settings, task_id, tenant_id),
    ...
    async def create_task(req: TaskRequest) -> TaskResult:
        ...
        store: SessionStore = app.state.session_store
        await store.create(
            name=req.title.strip() or task_id,
            session_id=task_id,
            tenant_id=req.tenant_id,
            graph_preset=req.graph or settings.graph_preset,
        )
    ...
    async def get_task(task_id: str, tenant_id: str = Query(DEFAULT_TENANT_ID)) -> TaskResult:
        await _check_tenant_owns_task(task_id, tenant_id)
        record = await app.state.session_store.get(task_id)
        graph_preset = record.graph_preset if record is not None else None
        service = await task_service_for(task_id, tenant_id, graph=graph_preset)
