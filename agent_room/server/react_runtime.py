...
2. **Shell allowlist (deny-by-default)** — `shell` only runs commands whose head
   token is in an explicit `settings.shell_allowlist` (default: disabled).
...
`tool_mode` (default `read_only`) decides whether write/shell survive the
...
_SAFE_WORKSPACE_SEGMENT = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$")
...
def task_workspace(settings: Settings, task_id: str, tenant_id: str = "default") -> Path:
    """Per-task subdir of the workspace, so tasks don't overwrite each other's
    files. Unsafe tenant/task path segments are rejected rather than sanitized
    into a potentially colliding directory."""
    if not _SAFE_WORKSPACE_SEGMENT.fullmatch(task_id):
        raise ValueError("task_id contains unsafe path characters")
    if not _SAFE_WORKSPACE_SEGMENT.fullmatch(tenant_id):
        raise ValueError("tenant_id contains unsafe path characters")
    root = workspace_dir(settings)
    ws = root / task_id if tenant_id == "default" else root / tenant_id / task_id
    ws.mkdir(parents=True, exist_ok=True)
    return ws
...
def build_server_registry(
    ...
    *,
    ...
    register_builtin_tools(
        ...
        sandbox_runner_url=settings.sandbox_runner_url,
        sandbox_runner_token=settings.sandbox_runner_token,
...
def build_server_spec(
    ...
    *,
    ...
    for node in data["nodes"].values():
        ...
        if settings.sandbox_runner_url and settings.sandbox_runner_token and "sandbox_exec" not in node["tools"]:
            node["tools"].append("sandbox_exec")
        if node["tools"]:
...
def describe_tool_envelope(
    ...
    return {
        ...
        "sandbox_runner_enabled": bool(
            settings.sandbox_runner_url and settings.sandbox_runner_token
        ),
    }
