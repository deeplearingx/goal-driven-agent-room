...
A multi-tool-call AIMessage produces one interrupt per call (LangGraph
matches resume values to interrupts by order in the node task). For now
the typical case — single tool call per turn — is the supported path;
multi-call approval is exercised in tests but the resume payload is then a
list of decisions, one per call.
