...
class Settings:
    ...
    # developer the built-in tools, confined to `workspace_dir`, with an
    # allowlisted shell. Empty `shell_allowlist` drops the shell tool entirely
    # (deny-by-default), and the server then drops `shell` from the spec too so
    # spec and registry stay consistent. See agent_room/server/react_runtime.py.
    graph_preset: str = "full_react"
    workspace_dir: str = "./workspace"
    shell_allowlist: tuple[str, ...] = ("pytest", "python", "python3", "ruff")
    tool_mode: str = "unrestricted"
...
def _parse_allowlist(raw: str | None) -> tuple[str, ...]:
    """Parse a comma-separated allowlist env var. Unset → conservative default;
    explicitly empty (``AGENT_ROOM_SHELL_ALLOWLIST=""``) → shell disabled."""
    if raw is None:
        return ("pytest", "python", "python3", "ruff")
...
def load_settings() -> Settings:
    ...
    return Settings(
        ...
        tool_mode=os.getenv("AGENT_ROOM_TOOL_MODE", "unrestricted"),
